  ...
  useEffect(() => {
    async function fetchLibrary() {
      try {
        ...
        if (data.success) {
          // Extract valid unique ebook objects from completed purchases
          const books = data.transactions
            .filter((t) => t.status === 'completed' && t.ebook)
            .map((t) => ({
              ...t.ebook,
              writer: t.writer // Ensure writer name is populated
            }));
  ...
  return (
    <div>
      <div style={{ marginBottom: '2.5rem' }}>
        ...
        <p style={{ color: 'var(--text-secondary)' }}>Instantly browse and read all your unlocked premium ebooks.</p>
      </div>
